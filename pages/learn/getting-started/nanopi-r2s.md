...
{% tabs %}
{% tab title="Flash" %}
If you have [Etcher](https://etcher.balena.io/) installed, you can just click the Flash button. This will automatically open Etcher with your image loaded. The contents of the image that gets loaded are the same as the contents of the image you would get from the **Download balenaOS** option. Once Etcher opens, proceed to [Provision device](nanopi-r2s.md#provision-device).
...
When complete, after a minute or two the device should appear on your balenaCloud [dashboard](https://dashboard.balena-cloud.com/), and you should now be ready to deploy some code. If you are not able get the device to appear on the dashboard, then check out our [troubleshooting guide for NanoPi R2S](https://github.com/balena-io/docs/blob/main/faq/troubleshooting/nanopi-r2s/README.md) or try our [support channels](../accounts/support-access.md).
